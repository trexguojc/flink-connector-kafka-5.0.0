...
public class KafkaSourceEnumerator
    ...
    /**
     * The splits that have been discovered during initialization but not assigned to readers yet. 这种应该很少见,没有分配的分区:任务失败等才有可能
     */
    ...
    public void start() {
        ...
        // These splits must not be reinitialized to keep offsets consistent with first discovery. 首次启动默认不走这里
    ...
    private PartitionSplitChange initializePartitionSplits(PartitionChange partitionChange) {
        ...
        Map<TopicPartition, Long> stoppingOffsets =
                newLinkedHashMapWithExpectedSize(newPartitions.size() + initialPartitions.size());
        if (!newPartitions.isEmpty()) { // 新发现的分区从最新的地方消费
            initOffsets(
                    newPartitions,
        ...
        if (!initialPartitions.isEmpty()) {// 迁移的分区(一开始失败了)按照制定的offset消费
            initOffsets(
    ...
    PartitionChange getPartitionChange(
        ...
        Consumer<TopicPartition> dedupOrMarkAsRemoved =// 只创建了一个lambda,下面才用到
                (tp) -> {
                    if (!newPartitions.remove(tp)) {
                        removedPartitions.add(tp);
                    }
                };
        // 遍历每个已经assign的分区,执行 dedupOrMarkAsRemoved.accept方法
